...
# %% {"scrolled": true}
T=np.ones((91, 91, 3), dtype=np.uint(8))
T=T*255
plt.imshow(T);
...
# %%
I=plt.imread('les-mines.jpg', format=None).copy()
I.flags.writeable
plt.imshow(I);
plt.show()
print("La dimension de l'image est", I.ndim)
print("La taille de l'image est", np.shape(I)[0:2])
print("le nombre d'octets par pixel est", I.itemsize)
print("le type des pixels est", I.dtype)
print("le min des pixels est", np.min(I))
print("le max des pixels est", np.max(I))
L=I[:11,:11]
plt.imshow(L);
...
# %%
def isole(l,c):
    plt.imshow(I[267-l//2:267+l//2+1,400-c//2:400+c//2+1])
isole(10,20)
...
plt.imshow(I[-1, -1].reshape(1, 1, 3))
...
# %%
I2=I.copy() 
I2[533-200:533,800-200:800]=(219,112,147)
plt.imshow(I2);
print("on obtient du rose")

# %%
I2[533-200:533,800-200:800]=(255,255,255)
I2[533-200:533:2,800-200:800]=(255,0,0)
plt.imshow(I2);
...
# %%
T=np.empty((533,800,4), dtype='uint8')
T[:,:,:3]=I
T[:,:,3]=128
plt.imshow(T);
...
# %% {"scrolled": true}
I=plt.imread('les-mines.jpg', format=None)
J=I.astype(float)/255
plt.imshow(J);
...
I=plt.imread('les-mines.jpg', format=None)
J=I.astype(float)
J=((0.299*J[:,:,0]+0.587*J[:,:,1]+0.114*J[:,:,2])/3)
plt.imshow(J, cmap='gray');
...
cor=I.astype(float)
cor=((0.299*J[:,:,0]+0.587*J[:,:,1]+0.114*J[:,:,2])/3)

racine=I.astype(float)/255

fig, axes = plt.subplots(1, 3)
axes[0,0].plot(moy)
axes[0,1].plot(cor)
axes[0,2].plot(racine)
plt.tight_layout()
